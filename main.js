...
document
  ...
  .addEventListener("click", clickCalculator);
...
function clickCalculator() {
  ...
  if (isNaN(amount) || amount <= 0) {
    ...
    document.getElementById("mortgage-amount-main");
    isValid = false;
  } else {
  ...
  if (isNaN(term) || term <= 0) {
    ...
    document.getElementById("mortgage-term-main");
    isValid = false;
  } else {
  ...
  if (isValid) {
    ...
    document.getElementById("result").innerText = `$${totalRepayment.toFixed(
      2
    )}`;
  } else {
